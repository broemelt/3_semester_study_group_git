...
test_inf_ID <- tibble(test_ID = test_informed$ID, test_pairID = test_informed$pair_ID)
...
test_informed_s <- cbind(test_inf_ID, test_informed_s)

test_informed_s <- subset(test_informed_s, select = -c(ID,pair_ID))

test_informed_s <- rename(test_informed_s, ID = test_ID)

test_informed_s <- rename(test_informed_s, pair_ID = test_pairID)
```
...
*note, we have made two models one with only the variables as predicters and one with random intercept because we expect the participants to have individual baselines. We do not have random slope because this assumes a linear development based on trial
```{r}
...
#### Discuss whether performance and feature importance is as expected
*note, would be ideal to run the model fit again after the feature selection but is is not neccessary (so lets not do this)
...
```{r}
sample_real <- sample(unique(real_data$PatID), 98)

train_data <- subset(real_data, PatID %in% sample_real)
test_data <- subset(real_data, !PatID %in% sample_real)

train_data$PatID <- as.factor(train_data$PatID)

test_data$PatID <- as.factor(test_data$PatID)

test_ID <- tibble(test_ID = test_data$PatID, test_newID = test_data$NewID, test_corp=test_data$Corpus)

train_corp <- tibble(train_corpus=train_data$Corpus)
...
test_data_s <- cbind(test_ID, test_data_s)
train_data_s <- cbind(train_corp, train_data_s)

test_data_s <- subset(test_data_s, select = -c(PatID,NewID, Corpus))
train_data_s <- subset(train_data_s, select = -c(Corpus))

train_data_s <- rename(train_data_s, Corpus=train_corpus)
test_data_s <- rename(test_data_s, PatID = test_ID)

test_data_s <- rename(test_data_s, newID = test_newID)

test_data_s <- rename(test_data_s, Corpus = test_corp)
```

### Principle component analysis
*note, check Lauras code
```{r}
...
```{r}
form1 <- bf(Diagnosis~1+(1|ID))
get_prior(form1, train_data_s, family = bernoulli)
...
```{r}
train_data_s$PredictionsPerc1 <- predict(pr_m1_fit)[, 1]
train_data_s$Prediction1[train_data_s$PredictionsPerc1 > 0.5] <- "Schizophrenia" 
train_data_s$Prediction1[train_data_s$PredictionsPerc1 <= 0.5] <- "Control" 

train_data_s <- train_data_s %>% 
  mutate(
    Diagnosis = as.factor(Diagnosis),
    Prediction1 = as.factor(Prediction1))

test_data_s$PredictionsPerc1 <- predict(pr_m1_fit, newdata = test_data_s, allow_new_levels = T)[, 1]
test_data_s$Prediction1[test_data_s$PredictionsPerc1 > 0.5] <- "Schizophrenia" 
test_data_s$Prediction1[test_data_s$PredictionsPerc1 <= 0.5] <- "Control" 
...
conf_mat(train_data_s, 
         truth = Diagnosis, 
         estimate = Prediction1, 
         dnn = c('prediction','truth'))

conf_mat(test_data_s, 
         truth = Diagnosis, 
         estimate = Prediction1, 
         dnn = c('prediction','truth'))
```

*note, It is okay that we predict by trial and not participant both is legitimate ways
