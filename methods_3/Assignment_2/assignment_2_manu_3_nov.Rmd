...
for (i in seq(nrow(sim_studies))){
    ...
    sim_studies$published[i] <- ifelse(abs(sim_studies$mean_effect_size)-(2*sim_studies$standard_error)>0 & sim_studies$mean_effect_size[i]>0, rbinom(1,1,0.9), rbinom(1,1,0.1))
  }
...
published_sim_studies <- filter(sim_studies, published==1)
```
