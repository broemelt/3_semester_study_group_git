...
```{r}
# loading packages
pacman::p_load(mousetrap, tidyverse, readbulk, ggthemes)

# loading in data
d <- read_opensesame(directory= 'exp_data_class')
```
...
m_square <- mt_subset(
  m,
  trial_type == 'rectangle'
)

m_elipse_high <- mt_subset(
  m,
  trial_type == 'elipse_high'
)
...
mt_plot(m_square, alpha = 0.5) +
  ggtitle('Square condition')

mt_plot(m_elipse, alpha = 0.5) +
  ggtitle('Elipse laying condition')

mt_plot(m_elipse_high, alpha = 0.5) +
  ggtitle('Elipse standing condition')
```
